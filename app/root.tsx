...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const [websiteRes, websiteErr] = await safeGet<any>(request, `https://cdn.revas.app/websites/v0/websites/facingmyfaces.davidegiovanni.com?public_key=01exy3y9j9pdvyzhchkpj9vc5w&language_code=it-IT`)
  if (websiteErr !== null) {
    throw new Error(`API website: ${websiteErr.message} ${websiteErr.code}`);
  }


  const fontFamilyUrl: string = websiteRes.website.theme.fontFamilyUrl
  const fontFamily: string = websiteRes.website.theme.fontFamily
